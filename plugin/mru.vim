...
function! s:MRU_Window_Edit_File(fname, multi, edit_type, open_type)
    ...
    elseif a:open_type ==# 'preview'
        ...
        " jump to it
        let fname = a:fname !~ '|$' ? a:fname : substitute(a:fname, '|$', '', 'g')
...
function! s:MRU_Open_Window(...)
    ...
    if a:0 && a:1 =~ '^:buffers'
      silent! let buflist = s:GetBufferInfo(a:1)
